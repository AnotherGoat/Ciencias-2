class MaquinaTuring {
    ...
    agrega_transicion(estado_, letra_, nuevoEstado_, escribe_, mueve_) {
        ...
        if (!this.estados.includes(estado_)) {
            console.log("Transición mal formada, estado ${estado} no permitido");
            return false;
        }
        // verifica movimiento
        if (mueve_ != "I" && mueve_ != "D") {
            console.log("Transición mal formada, movimiento debe ser I o D");
            return false;
        }
    ...
    mueve(I_o_D) {
        if (I_o_D == "I" && this.cabezal > 0)
            this.cabezal--;
        // Correción para transiciones # en el estado inicial, cuando se empieza con la palabra vacía ""
        else if (I_o_D == "D" && this.cabezal < this.cinta.length && this.cinta.length !== 2)
            this.cabezal++;
        else if (I_o_D == "I")
